 ...
 * so draft2final can load them as defaults at runtime without compile-time coupling.
 */
...
const d2fRoot = path.resolve(__dirname, '..');
const repoRoot = path.resolve(d2fRoot, '..');

const destinations = [
    {
        src: path.join(repoRoot, 'contexts', 'pdf', 'dist'),
        dest: path.join(d2fRoot, 'dist', 'bundled', 'contexts', 'pdf'),
        label: 'contexts/pdf'
    },
    {
        src: path.join(repoRoot, 'font-managers', 'local', 'dist'),
        dest: path.join(d2fRoot, 'dist', 'bundled', 'font-managers', 'local'),
        label: 'font-managers/local'
    },
    {
        src: path.join(repoRoot, 'font-managers', 'local', 'assets'),
        dest: path.join(d2fRoot, 'dist', 'bundled', 'font-managers', 'assets'),
